...
export default function Sidebar({ 
  ...
  // Component updated to match the usage in App.tsx which handles tab switching and PWA installation
  return (
    <nav className="fixed z-50 bg-zinc-950/80 backdrop-blur-2xl border-zinc-800 bottom-0 w-full h-auto flex flex-row items-center justify-around py-3 border-t md:left-0 md:top-0 md:bottom-0 md:w-20 md:flex-col md:justify-start md:py-10 md:gap-8 md:border-r md:border-t-0">
      ...
      <div className="flex md:flex-col gap-6 items-center">
        ...
        <button
          ...
          className={`relative p-3 rounded-xl transition-all group ${activeTab === GenerationType.CONTRACT ? 'bg-amber-400 text-black shadow-lg shadow-amber-400/20' : 'bg-amber-400/10 text-amber-400 border border-amber-400/20'}`}
        ...
        {installPrompt && (
          <button
            ...
            className="p-3 rounded-xl bg-green-500/10 text-green-500 border border-green-500/20 hover:bg-green-500/20 transition-all group relative"
